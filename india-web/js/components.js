    ...
    for (key in data) {
      if (key.startsWith('validate')) {
        ...
        if (!validatorPattern.test($el.val())) {
          $validatorCheckbox.toggleClass('india-checklist-checked', false);
        }
        else {
          $validatorCheckbox.toggleClass('india-checklist-checked', true);
        }
      }
    }
  }
...
$(function() {
  $('[class^=india-accordion]').each(function() {
    accordion($(this));
  });
  ...
  var footerAccordion = function() {
    if (window.innerWidth < 600) {
      ...
      $('.india-footer-big nav ul').addClass('hidden');

      $('.india-footer-big nav .india-footer-primary-link').unbind('click');

      $('.india-footer-big nav .india-footer-primary-link').bind('click', function() {
        $(this).parent().removeClass('hidden')
        .siblings().addClass('hidden');
      });
    } else {

      $('.india-footer-big nav ul').removeClass('hidden');
  ...
  togglePassword($('.india-show_password'));
  toggleMultiPassword($('.india-show_multipassword'));
  toggleSSN($('.india-show_ssn'));
